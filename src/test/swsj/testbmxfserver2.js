...
function doCopyLastMonthDatas(args){
	...
	if (isAnalyse){
		...
		//查询是否在当前的表中已经下发过了数据
		var sql = "select t0.XMMC,t0.XMBH,t0.SSSCQY,t0.CDBM from "+ f +" t0 where " +
				"exists(select 1 from " + dataSource + " where  XMMC=t0.XMMC and " +
				"XMBH=t0.XMBH and SSSCQY=t0.SSSCQY and CDBM=t0.CDBM and BBQ=?) and  t0.BBQ=?"
		print("copyedSql--------" + sql);		
		...
		var sql = "select t0.* from " + f +" t0 where " +
				...
				"and SSSCQY=t0.SSSCQY and CDBM=t0.CDBM and BBQ=?) and  BBQ=?"
	...
	if (hasCopied != ""){
		...
		if(!isDQ){
			var sql = "insert into "+ f +"(" +
					   "XMMC,XMBH,SSSCQY,CDBM,XMJZZT,XMFZR," +
				"BBQ) " +
				"select XMMC,XMBH,SSSCQY,CDBM,XMJZZT,ZCDFZR," +
				"? as BBQ from "+ dataSource +" t0 where " +
				"not exists(select 1 from "+ dataSource +" where XMMC=t0.XMMC and XMBH=t0.XMBH and SSSCQY=t0.SSSCQY and" +
				" CDBM=t0.CDBM and BBQ=?) and  BBQ=?";
			print(sql);
			...
			var sql = "insert into "+ f +"(" +
					   "XMMC,XMBH,SSSCQY,CDBM,XMJZZT,ZCDFZR,XMZYS_YSRWE,XMZYS_YSCGWXCB,XMZYS_ZMSR,QNDYJSWGE,QNDYJSMSR," +
				"BNRWMSRZB_SCRWE,BNRWMSRZB_SCMSR,BNRWMSRZB_SJFPMSR,HTZXQK_YQCGHTE,HTZXQK_YQXSHTE,BBQ) " +
				"select XMMC,XMBH,SSSCQY,CDBM,XMJZZT,ZCDFZR,XMZYS_YSRWE,XMZYS_YSCGWXCB,XMZYS_ZMSR,QNDYJSWGE,QNDYJSMSR," +
